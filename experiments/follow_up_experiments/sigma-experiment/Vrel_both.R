...
for (sigma_folder in sigma_folders) {
  ...
  vrel_array_procrustes <- numeric(length(sim_files))
  vrel_array_noalign <- numeric(length(sim_files))
  ...
  for (i in seq_along(sim_files)) {
    ...
    vrel_array_procrustes[i] <- integration_measure_procrustes$Re.obs

    # Non-aligned integration measure
    integration_measure_noalign <- integration.Vrel(sim_mat)
    vrel_array_noalign[i] <- integration_measure_noalign$Re.obs
  }

  integration_measures_list_procrustes[[sigma_value]] <- vrel_array_procrustes
  integration_measures_list_noalign[[sigma_value]] <- vrel_array_noalign
}
